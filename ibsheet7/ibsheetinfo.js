...
function createIBSheet(sheetid, width, height, locale) {
	...
	window[sheetid].SetDown2ExcelConfig({
		"FileName":"Excel.xlsx"
	   ,"ExcelFontSize":11
	   ,"ExcelRowHeight":22
	   ,"KeyFieldMark":false
...
//메서드 override
function DoSaveMethodOverride(id) {
	window[id].DoSaveOrg = window[id].DoSave;
	
	//재정의한다
	window[id].DoSave = function(PageUrl, Opt) {
		//해당 시트의 저장 확인용 메세지 변경처리(공통)
		var ss  = "입력 건수 는 " + window[id].RowCount("I") + " 건, \n";
		    ss += "수정 건수 는 " + window[id].RowCount("U") + " 건, \n";
			ss += "삭제 건수 는 " + window[id].RowCount("D") + " 건입니다.\n";
			ss += "저장하시겠습니까?";
		
		window[id].SetMessageText("SaveConfirm", ss);
		
		//opt이 없거나, opt이 있는경우 quest는 무조건 1로 변경
