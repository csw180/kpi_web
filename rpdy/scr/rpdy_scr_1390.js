...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "조회":             // 조회(상위 GRID)
            ...
            if (pgcode != '16') {
                mySheet.DoSearch("rpdy.scr.rpdy_scr_1390.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=1"+"&metrogb="+metrogb);//xwp_rpdy_N일일주거래상품판매실적프로시져사용(구분코드3" );
...
function initGrid()
{
	...
	else /* else 그리드가 존재하지 않아 추가 2022-02-09 오진아 */
		if(baseday != "" ) {
			alert("해당 일자는 지원하지 않습니다.");
			document.frm.baseday.focus();
			return;
		}
...
function setGrid2022H2()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
    var strHeader0 = "점번호|점명|PG명|신규고객|기존고객|자동이체|교차판매|최종실적\n(E)=(A+B+C+D)|목표\n(F)|달성률\n(G)=(E/F)|";
...
function setGrid2022H()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
    var strHeader0 = "점번호|점명|PG명|신규고객|기존고객|최종실적\n(C)=(A+B)|목표\n(D)|달성률\n(E)=(C/D)|";
	var strHeader1 = "점번호|점명|PG명|실적\n(A)|실적\n(B)|최종실적\n(C)=(A+B)|목표\n(D)|달성률\n(E)=(C/D)|";
...
function setGrid2021H2()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
   ...
   var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   var headers = [ { Text:"점번호|점명|PG명|신규고객\n실적(A)|기존고객\n실적(B)|경리나라\n추가실적(C)|펌뱅킹\n추가실적(D)|최종실적\n(E)=(A+B+C+D)|목표\n(F)|달성률\n(G)=(E/F)|", Align:"Center"} ];
   mySheet.InitHeaders(headers, info);
...
function setGrid2020H2()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
   ...
   var cols = [ {Type:"Text",     Hidden:0,  MinWidth:50,   Align:"Center",  ColMerge:1,   SaveName:"NONE" },
             ...
             {Type:"AutoSum",   Hidden:0, MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"AutoSum",   Hidden:0, MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"Float",     Hidden:0,  MinWidth:80,   Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"|7|/|8|*100",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 } ];
...
function setGrid2020()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
   ...
   var cols = [ {Type:"Text",     Hidden:0,  MinWidth:50,   Align:"Center",  ColMerge:1,   SaveName:"NONE" },
             ...
             {Type:"AutoSum",   Hidden:0, MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"AutoSum",   Hidden:0, MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"Float",     Hidden:0,  MinWidth:80,   Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"|5|/|6|*100",   Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 } ];
