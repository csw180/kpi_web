...
<%  
    ...
    String      leftmenu_name    = "저비용성증대 실적"; 
    String      htm_title        = "저비용성증대 실적"; 
    String      actionname       = "";
    String      haengwon_no      = "";
    String      strErrMsg        = "";
    String      status           = "0";
...
<script language="javascript" src="/kpi/rpdy/scr/rpdy_scr_1450.js"></script>
...
function hiddGridmon_OnSearchEnd(Code, Msg){
    ...
    if(isRoleEnable == '1') {
    	 // 전산정보부 및 성과 담당자
         hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1450.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=98");
    } else {
         hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1450.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=99");
    }
}
...
function mySheet_OnSearchEnd(Code, Msg){
	...
	if (document.frm.baseday.value >= '20220701') {
    	...
    	mySheet.SetCellValue(mySheet.HeaderRows(), 14, (mySheet.GetCellValue(mySheet.HeaderRows(), 12) / mySheet.GetCellValue(mySheet.HeaderRows(), 13) * 100).toFixed(1) ,0);
    } 
}  
