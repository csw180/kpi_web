...
function LoadPage(){
  ...
  createIBSheet2(  document.getElementById("hiddenGridObj"),"hiddenGrid", "200px", "400px");   // 기본정보 (기준일, 명칭등)   // 기본정보 (기준일, 명칭등)
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준일":           // 조회(상위 GRID)
            ...
            if(isRoleEnable == '1') {
               // IT담당자 및 성과 담당자
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1381.do?hidden_key=9&baseday="+baseday+"&sergb=98");
            } else {
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1381.do?hidden_key=9&baseday="+baseday+"&sergb=99");
            }*/
            break;
        case "조회":             // 조회(상위 GRID)
            //if(baseday >= '20200101') {
            ...
            mySheet.DoSearch("rpdy.scr.rpdy_scr_1381.do?hidden_key=9&baseday="+baseday+"&sergb=1&jumcode="+jumcode+"&searchCode="+searchCode+"&pg_url="+pg_url);
...
function setGrid2022()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
	var strHeader0 = "고객번호|고객명|고객구분|1.원화예수금|1.원화예수금|1.원화예수금|"
         			 +"2.외화예수금|2.외화예수금|2.외화예수금|3.펀드|4.공제|5.신탁|최종실적\n(J)=(C+F+G+H+I)|최종\n포인트";

	var strHeader1 = "고객번호|고객명|고객구분|반기평잔\n(A)|잔액\n(B)|평가금액\n(C)=MAX(A,B)|"
         			 +"반기평잔\n(D)|잔액\n(E)|평가금액\n(F)=MAX(D,E)|잔고원금\n(G)|공제료\n(H)|잔액\n(I)|최종실적\n(J)=(C+F+G+H+I)|최종\n포인트";
    ...
    var cols = [ {Type:"Text",     Hidden:0,  MinWidth:150,   Align:"Center",  ColMerge:1,   SaveName:"NONE" },
...
function setGrid2020H2()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
   ...
   var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   var headers = [ { Text:"고객번호|고객명|고객구분|잔액|반기평잔|평가금액|고객평가점수", Align:"Center"} ];
   mySheet.InitHeaders(headers, info);
