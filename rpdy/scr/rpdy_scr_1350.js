...
function LoadPage(){
  ...
  // Hidden GRID에 대한 속성정보 설정
   ...
   var hdnCols = [ {Type:"Text",      Hidden:1, MinWidth:0,    Align:"Center",  SaveName:"0",  CalcLogic:"",   Format:"",            PointCount:0,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Text",      Hidden:1, MinWidth:0,    Align:"Center",  SaveName:"0",  CalcLogic:"",   Format:"",            PointCount:0,  UpdateEdit:0,   InsertEdit:0 } ];
...
//metrogbhiddenGrid에 대한 속성정보 설정
   ...
   var mtrhdnCols = [ {Type:"Text",      Hidden:1, MinWidth:0,    Align:"Center",  SaveName:"0",  CalcLogic:"",   Format:"",            PointCount:0,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Text",      Hidden:1, MinWidth:0,    Align:"Center",  SaveName:"0",  CalcLogic:"",   Format:"",            PointCount:0,  UpdateEdit:0,   InsertEdit:0 } ];
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준일":           // 조회(상위 GRID)
            ...
            if(isRoleEnable == '1') {
             // 전산정보부 및 성과 담당자
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1350.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=98");
            } else {
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_1350.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=99");
            }
            break;
        case "조회":             // 조회(상위 GRID)
//            if(baseday >= '20180701') {
...
//            }
            if (pgcode != '16') {
                mySheet.DoSearch("rpdy.scr.rpdy_scr_1350.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb="+sergb+"&metrogb="+metrogb);
                //mySheet.FitSize(false, true);
            } else {
                alert("금융본부는 저비용성예수금 실적(금융본부) 에서 확인하시기 바랍니다.");
            }
            break;
        case "엑셀내려받기":    // 엑셀내려받기
            inqText=new Array();
...
function setGrid2021H() 
{
  ...
  // 상위 GRID에 대한 속성정보 설정
   ...
   var cols = [ {Type:"Text",     Hidden:0,  MinWidth:60,   Align:"Center",  ColMerge:1,   SaveName:"NONE" },
             ...
             {Type:"AutoSum",   Hidden:0, MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"Integer",     PointCount:0,   DecimalAdjust:1, UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"AutoSum",   Hidden:0, MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"Integer",     PointCount:0,   DecimalAdjust:1, UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
             {Type:"Float",     Hidden:0,  MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",  CalcLogic:"|23|/|24|*100",Format:"NullFloat",   PointCount:1,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 } ];
...
function setGrid2020() // lhi
{
  ...
  // 상위 GRID에 대한 속성정보 설정
	...
	strHeader1 = "점번호|점명|PG명|총금액(A)|"
      +"소계|금융기관성|RM(본부)실적|RM(영업점)실적|연구위원|타지점|본부조달|구속성예금|임직원가족|단기신규해지|본부마케팅|독도상품실적|평가제외|기타조정|"                    // 조정내역(14)
      +"누증실적\n(C)=(A+B)|순수저비용성\n추가인정(D)|외화예수금\n순증실적(E)|최종 누증실적\n(F)=(C+D+E)|조정전\n기준실적(G)|조정후\n기준실적(H)|순증실적\n(I)=(F-H)|목표\n(J)|달성률\n(K)=(I/J)|";
