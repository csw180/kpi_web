...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준일":           // 조회(상위 GRID)
            ...
            if(isRoleEnable == '1') {
             // 전산정보부 및 성과 담당자
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_2410.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=98");               
            } else {
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_2410.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=99");
            }
			*/
            break;
        case "조회":             // 조회(상위 GRID)
...
//            }
            if (pgcode != '16') {
                mySheet.DoSearch("rpdy.scr.rpdy_scr_2410.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb="+sergb+"&metrogb="+metrogb+"&kngb=1"); 
            } else {
                alert("금융본부는 영업점 개인핵심예수금 실적(금융본부) 에서 확인하시기 바랍니다.");
            }
            break;
        case "엑셀내려받기":    // 엑셀내려받기
            inqText       = new Array();
