...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준일":           // 조회(상위 GRID)
            ...
            if(isRoleEnable == '1') {
            	 // 전산정보부 및 성과 담당자
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_2300.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=98");
            } else {
               hiddenGrid.DoSearch("rpdy.scr.rpdy_scr_2300.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&sergb=99");
            }*/
            break;
        case "조회":             // 조회(상위 GRID)
           ...
           // }        
            if (baseday >= '20210101')
            { //2021년 상반기 부터 기업부지점장 a,b 구분
            	mySheet.DoSearch("rpdy.scr.rpdy_scr_2300.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&metrogb="+metrogb+"&sergb=1" );
            }
            else {
				mySheet.DoSearch("rpdy.scr.rpdy_scr_2300.do?hidden_key=9&baseday="+baseday+"&pgcode="+pgcode+"&metrogb="+metrogb);
			}
...
function setGrid2021H()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
	...
	strHeader2 =  
	  ...
	  +"목표|실적|달성률|평점/배점|"			          // 4 기업저비용성예수(반기평잔)
	  +"목표|실적|달성률|평점/배점 |" 			        // 4 기업저비용성예수(월중평잔)	 
	  +"밴드범위|실적|밴드범위이탈금액|평점/배점 |"  // 4 대출금(순증잔액_밴드평가)
...
function setGrid2020H2()
{
  ...
  // 상위 GRID에 대한 속성정보 설정
	...
	strHeader1 = 
	  ...
	  //개인실적 5
	  +"기업여신신규금액|기업여신신규금액|기업여신신규금액|기업여신신규금액|기업여신신규금액|"                                     
	  ;
	...
	strHeader2 = 
	  ...
	  //개인실적 5
	  +"목표|실적|달성률|평점/배점|그룹내순위|"               // 5 기업여신신규금액	      
	  ;
