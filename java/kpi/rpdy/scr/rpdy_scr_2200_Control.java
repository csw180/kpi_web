...
/**
 *  클래스 개요     : 후선보임 권유계좌리스트 조회
 *  클래스 상세기능 : 후선보임 권유계좌리스트를 읽어서 JSP에 전달한다.
 *  작성일 : 2013.11.20 
 *  @version 1.0
 *  @author  조형석
 */
public class rpdy_scr_2200_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            rs = jado.callProc("UP_KPI_D_S_일일후선보임권유리스트", baseday +","+ emnm+","+sergb);
            ...
            if (!(sergb.equals("99") || sergb.equals("98"))){  /***** 기준일 조회 *****/
                ...
                jado.InputProc("UP_KPI_C_S_WEBLOGN"
                              ,"1"                       +","  //1:일일성과보고서, 2:영업점보고서, 3:개인PI, 5:운영관리
                              +"일일후선보임권유리스트"  +","  //프로그램ID
                              +startTime                 +","  //시작시간
                              +endTime                   +","  //완료시간
