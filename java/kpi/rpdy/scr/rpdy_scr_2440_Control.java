...
/**
 *  클래스 개요     : 영업점 일일 기업금융지점장 저비용성증대 실적
 *  클래스 상세기능 : 영업점 일일 기업금융지점장 저비용성증대 실적 내역을 읽어서 JSP에 전달한다. 
 *  작성일 : 2022.09.07
 *  @version 1.0
 *  @author  이해인
 */
public class rpdy_scr_2440_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String haeksgb    = JSPUtil.getParameter(request, "haeksgb"," ");
        ...
        try
        {
            ...
            rs = jado.callProc("UP_KPI_D_S_N일일RM저비용성증대실적", baseday+","+pgcode+","+sergb+","+metrogb+","+haeksgb); // 2022년 일일주거래신규우량고객실적
