...
/**
 *  클래스 개요     : 영업점 일일 수시입출식실적 인정 조회
 *  클래스 상세기능 : 영업점 일일 수시입출식실적 인정을 읽어서 JSP에 전달한다.
 *  작성일 : 2013.03.07
 *  @version 1.0
 *  @author  조형석
 */
public class rpdy_scr_2020_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            baseMon  = Integer.parseInt(baseday);

            //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();

            rs = jado.callProc("UP_KPI_D_S_N일일수시입출식인정실적", baseday+","+pgcode+","+sergb+","+metrogb+","+fundgb);
