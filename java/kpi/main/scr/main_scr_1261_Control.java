...
/**
 *  클래스 개요     : 점별연체금액조정 조회 및 처리  <p>
 *  클래스 상세기능 : 점별연체금액조정 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1261_Control extends HTMLActionSupport   
{
    /**
     * HttpRequst의 정보를 점별연체금액조정 조회 데이타모델로 파싱하여 반환한다.
     */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_점별연체금액조정조회",basemonth);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_점별연체금액조정삭제",basemonth);
                ...
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"    ,0); //고객구분
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"    ,0); //고객번호              
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_gubun"   ,0); //연체대상구분
                String v_sv_6[]  = JSPUtil.getParameter(request,"v_sv_6"    ,0); //차감연체금액                               
                String  strEmpNo     = account.getHaengwon_no();
                
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_점별연체금액조정처리",  v_sv_1[i]  + "," +
                                                                        v_sv_2[i]  + "," +
                                                                        v_sv_3[i]  + "," +
                                                                        v_sv_4[i]  + "," +
                                                                        v_sv_5[i]  + "," +
                  ...
                  jado.InputProc("UP_KPI_C_S_WEBLOG"
                                ...
                                +"고객구분="         +v_sv_3[i]              + ";" //고객구분
                                +"고객번호="         +v_sv_4[i]              + ";" //고객번호
                                +"연체대상구분="     +v_sv_5[i]                + ";" //연체대상구분
                                +"차감연체금액="     +v_sv_6[i]              + ";" //차감연체금액
                                );
                                                                      
                  //System.out.println( v_sv_1[i]  + "," +  v_sv_2[i]  + "," + v_sv_3[i] );
