...
/**
 *  클래스 개요     : 실적조정계좌 등록 조회 및 처리  <p>
 *  클래스 상세기능 : 실적조정계좌 등록 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2012.04.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1291_Control extends HTMLActionSupport   
{
    /**
     * HttpRequst의 정보를 실적조정계좌 데이타모델로 파싱하여 반환한다.
     */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String adgubun    = JSPUtil.getParameter(request,"adgubun"," ");
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_실적조정계좌",basemonth+ "," + adgubun);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_실적조정계좌삭제",basemonth + "," + adgubun);
                ...
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"    ,0);  //계정과목코드
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"    ,0);  //고객번호
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"    ,0);  //참고사항1 (해투금 동일계좌번호)
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"    ,0);  //참고사항2 (목표30%인정제외여부)
                ...
                for(int i = 0; i < sStatus.length; i++) {
                	/*
                  	rc = jado.InputProc("UP_KPI_O_S_실적조정계좌처리",  v_sv_1[i]  + "," +
                                                                    adgubun    + "," +
                                                                    v_sv_2[i]  + "," +
                                                                    v_sv_3[i]  + "," +
                                                                    v_sv_4[i]  + "," +
					...
					*/
                	...
                	tmpParam += v_sv_1[i]  + "," +
                            adgubun    + "," +
							("".equals(v_sv_2[i]) || v_sv_2[i] == null ? " " : v_sv_2[i])  + "," +
                            v_sv_3[i]  + "," +
                            ("".equals(v_sv_4[i]) || v_sv_4[i] == null ? " " : v_sv_4[i])  + "," + 
                            v_sv_5[i]  + "," +
                            v_sv_7[i]  + "," +
                            v_sv_8[i]  + "," +
                            v_sv_9[i]  + "," +
                  ...
                  // Error message 값 SET 위한 index -------------------------->
        			...
        			if(i==0) tmpQuery = jado.InputProcS("UP_KPI_O_S_실적조정계좌처리", tmpParam);
