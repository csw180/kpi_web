...
/**
 *  클래스 개요     : RMC인정 개인고객 관리 <p>
 *  클래스 상세기능 : RMC인정개인고객 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1231_Control extends HTMLActionSupport
{
      /**
       * HttpRequst의 정보를 RMC인정개인고객 조회 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_RMC인정개인고객조회");
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_RMC인정개인고객처리","4,0,0,0" );
                ...
                String v_sv_1[]  = JSPUtil.getParameter(request,"v_sv_1"   ,0); //고객번호
                String v_sv_2[]  = JSPUtil.getParameter(request,"v_sv_2"   ,0); //고객명
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"   ,0); //일정대상구분
                String strEmpNo  = account.getHaengwon_no();

                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_RMC인정개인고객처리","1," +
                                                           v_sv_1[i]  + "," +
                                                           v_sv_3[i]  + "," +
                                                           strEmpNo       
                                                                  );
                  ...
                  jado.InputProc("UP_KPI_C_S_WEBLOG"
                                ...
                                +account.getBranch_no()              +","  //점번호
                                +request.getRemoteAddr()             +","  //사용자IP
                                +"등록 : 고객번호="  +v_sv_1[i]              + ";" //고객번호
                                +"인정대상구분="     +v_sv_3[i]              + ";" //인정대상구분
                                );
