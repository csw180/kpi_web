...
/**
 *  클래스 개요    : 영업점 팀장 관리 조회 및 처리
 *  클래스 상세기능: 영업점 팀장 관리 upload 처리 및 조회하여 JSP에 전달한다.
 *  작성일 : 2020.02.19
 *  @version 1.0
 *  @author  양인찬
 */
public class main_scr_5511_Control extends HTMLActionSupport
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String jumcode     = JSPUtil.getParameter(request, "jumcode" ," ");     // 점번호
        String posi_gubun  = JSPUtil.getParameter(request, "posi_gubun" ," ");  // 직위구분
        String team_gubun  = JSPUtil.getParameter(request, "team_gubun" ," ");  // 팀구분
        String emnm        = JSPUtil.getParameter(request, "emnm" ," ");        // 직원번호
        String stdate      = JSPUtil.getParameter(request, "stdate" ," ");      // 발령일
        String enddate     = JSPUtil.getParameter(request, "enddate" ," ");     // 종료일
        String v_note      = JSPUtil.getParameter(request, "v_note" ," ");      // 비고
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_영업점팀장관리조회",basemonth+","+posi_gubun);
               //조회종료시간
               ...
               rc = jado.InputProc("UP_KPI_O_S_영업점팀장관리처리","1,"   +
                                                           basemonth      + "," +
                                                           jumcode        + "," +
                                                           posi_gubun     + "," +
                                                           team_gubun     + "," + 
               ...
               String old_posi_gubun  = JSPUtil.getParameter(request, "old_posi_gubun" ," ");      // 구직위구분
               String old_team_gubun  = JSPUtil.getParameter(request, "old_team_gubun" ," ");      // 구팀구분
               ...
               rc = jado.InputProc("UP_KPI_O_S_영업점팀장관리처리","2,"   +
                                                           basemonth      + "," + 
                                                           jumcode        + "," + 
                                                           posi_gubun     + "," + 
                                                           team_gubun     + "," +
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                             ...
                             +"(구)직위구분="        +old_posi_gubun  + ";" //직위구분
                             +"(구)팀구분="          +old_team_gubun  + ";" //팀구분
               ...
               rc = jado.InputProc("UP_KPI_O_S_영업점팀장관리처리","3,"   +
                                                           basemonth      + "," +
                                                           jumcode        + "," + 
                                                           posi_gubun     + "," + 
                                                           team_gubun     + "," +
               ...
               String[] jumcodeA    = JSPUtil.getParameter(request, "v_sv_1",  0);  // 점번호  
               String[] posi_gubunA = JSPUtil.getParameter(request, "v_sv_6",  0);  // 직위구분  
               String[] team_gubunA = JSPUtil.getParameter(request, "v_sv_7",  0);  // 팀구분         
               String[] emnmA       = JSPUtil.getParameter(request, "v_sv_8",  0);  // 직원번호
               String[] stdateA     = JSPUtil.getParameter(request, "v_sv_10",  0);  // 발령일
               String[] enddateA    = JSPUtil.getParameter(request, "v_sv_11",  0);  // 종료일
               String[] v_noteA     = JSPUtil.getParameter(request, "v_sv_12",  0);  // 비고

               // 작업기준년월로 전체 테이타 삭제
               rc = jado.InputProc("UP_KPI_O_S_영업점팀장관리처리","4,"       +
                                                           basemonth          + "," +
                                                           jumcodeA[0]        + "," +
                                                           posi_gubunA[0].replaceAll(" ", "").substring(0, 5)     + "," +
                                                           team_gubunA[0]     + "," +
                                                           emnmA[0]           + "," +
                                                           stdateA[0]         + "," +
                                                           enddateA[0]        + "," +
               ...
               for(int i=0; i<strStatus.length; i++) {
                   ...
                   rc = jado.InputProc("UP_KPI_O_S_영업점팀장관리처리","1,"   +
                                                           basemonth          + "," + 
                                                           jumcodeA[i]        + "," +
                                                           posi_gubunA[i].replaceAll(" ", "").substring(0, 5)     + "," +
                                                           team_gubunA[i]     + "," +
                                                           emnmA[i]           + "," +
                                                           stdateA[i]         + "," +
                                                           enddateA[i]        + "," +
