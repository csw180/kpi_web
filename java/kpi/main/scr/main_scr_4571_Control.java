...
/**
 *  클래스 개요     : 부지점장 고객관리  <p>
 *  클래스 상세기능 : 부지점장 고객관리 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2015.03.19<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_4571_Control extends HTMLActionSupport   
{
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_부지점장고객관리조회",basemonth+","+jumcode);
                //조회종료시간
                ...
                if (hidden_key.equals("1")){
                    rc = jado.InputProc("UP_KPI_O_S_부지점장고객관리삭제", basemonth);
                    //삭제종료시간
                    endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                    //web log 생성 
                    jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_부지점장고객관리처리", hidden_key + "," +
                                                                v_sv_1[i]   + "," + 
                                                                v_sv_2[i]   + "," + 
                                                                v_sv_3[i]   + "," + 
                                                                v_sv_4[i]   + "," + 
