 ...
 *  @author  하진영
 */
public class main_scr_3031_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 영업점배점 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String v_jumcode  = JSPUtil.getParameter(request, "v_jumcode" ,"0000");
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                ...
                rc = jado.InputProc("UP_KPI_O_S_영업점배점삭제", basemonth);
                //조회종료시간
                endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                ...
                for(int i = 0; i < sStatus.length; i++) {
                	...
                	/*
                	rc = jado.InputProc("UP_KPI_O_S_영업점배점처리",  v_sv_1[i]  + "," + 
                  	                                              v_sv_2[i]  + "," + 
                  	                                              v_sv_3[i]  + "," + 
                  	                                              v_sv_4[i]  + "," + 
                  	                                              v_sv_5[i]  + "," + 
        			...
        			if(i==0) tmpQuery = jado.InputProcS("UP_KPI_O_S_영업점배점처리", tmpParam);
