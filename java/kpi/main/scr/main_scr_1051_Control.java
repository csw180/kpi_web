...
/**
 *  클래스 개요    : 기타조정손익등록  <p>
 *  클래스 상세기능 : 기타조정손익 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2011.01.01<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1051_Control extends HTMLActionSupport
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_기타조정손익",basemonth+","+v_jumcode+",0");
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_기타조정손익삭제",basemonth);
                ...
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"  ,0);  //조정구분
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"  ,0);  //평가구분
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"  ,0);  //조정금액 
                String  strEmpNo     = account.getHaengwon_no();
                
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_기타조정손익처리",  v_sv_1[i]  + "," +
                                                                    v_sv_2[i]  + "," +
                                                                    v_sv_3[i]  + "," +
                                                                    v_sv_4[i]  + "," +
                                                                    v_sv_5[i]  + "," +
