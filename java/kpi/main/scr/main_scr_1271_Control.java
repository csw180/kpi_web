...
/**
 *  클래스 개요    : 기타실적조정계좌 등록  <p>
 *  클래스 상세기능 : 기타실적조정계좌 등록 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2014.03.27<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_1271_Control extends HTMLActionSupport
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_기타실적조정계좌",basemonth);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                // 자료삭제 
                rc = jado.InputProc("UP_KPI_O_S_기타실적조정계좌처리","4,"+basemonth+",0,0,0,0,0");
                ...
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_기타실적조정계좌처리",  "1,"+ v_sv_1[i]  + "," +
                                                                              v_sv_2[i]  + "," +
                                                                              v_sv_3[i]  + "," +
                                                                              v_sv_4[i]  + "," +
                                                                              v_sv_5[i]  + "," +
