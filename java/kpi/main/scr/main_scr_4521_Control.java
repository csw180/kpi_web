...
/**
 *  클래스 개요     : 부지점장목표  <p>
 *  클래스 상세기능 : 부지점장목표 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2014.07.21<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_4521_Control extends HTMLActionSupport   
{
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_부지점장목표조회",basemonth+","+jumcode);
                //조회종료시간
                ...
                if (hidden_key.equals("1")){
                    rc = jado.InputProc("UP_KPI_O_S_부지점장목표삭제", basemonth);
                    //삭제종료시간
                    endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                    //web log 생성 
                    jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"    ,0);   //연간목표
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"    ,0);   //1st월
                String v_sv_9[]  = JSPUtil.getParameter(request,"v_sv_9"    ,0);   //2nd월
                String v_sv_10[] = JSPUtil.getParameter(request,"v_sv_10"   ,0);   //3rd월
                String v_sv_11[] = JSPUtil.getParameter(request,"v_sv_11"   ,0);   //4th월
                String v_sv_12[] = JSPUtil.getParameter(request,"v_sv_12"   ,0);   //5th월
                String v_sv_13[] = JSPUtil.getParameter(request,"v_sv_13"   ,0);   //6th월
                String  strEmpNo     = account.getHaengwon_no();                   // 직원번호
                for(int i = 0; i < sStatus.length; i++) {
                rc = jado.InputProc("UP_KPI_O_S_부지점장목표처리", hidden_key + "," +
                                                                v_sv_1[i]   + "," + 
                                                                v_sv_2[i]   + "," + 
                                                                v_sv_3[i]   + "," + 
                                                                v_sv_4[i]   + "," + 
