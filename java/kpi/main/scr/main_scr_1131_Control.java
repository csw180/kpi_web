...
/**
 *  클래스 개요     : 기준실적 조회 및 처리  <p>
 *  클래스 상세기능 : 기준실적 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1131_Control extends HTMLActionSupport   
{
    /**
     * HttpRequst의 정보를 기준실적 데이타모델로 파싱하여 반환한다.
     */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_기준실적",basemonth+","+v_jumcode);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_기준실적삭제",basemonth);
                ...
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"    ,0);  //kpi코드
                ...
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"    ,0);  //반기1st월기준실적
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"    ,0);  //반기2nd월기준실적
                String v_sv_9[]  = JSPUtil.getParameter(request,"v_sv_9"    ,0);  //반기3rd월기준실적
                String v_sv_10[] = JSPUtil.getParameter(request,"v_sv_10"   ,0);  //반기4th월기준실적
                String v_sv_11[] = JSPUtil.getParameter(request,"v_sv_11"   ,0);  //반기5th월기준실적
                String v_sv_12[] = JSPUtil.getParameter(request,"v_sv_12"   ,0);  //반기6th월기준실적
                ...
                for(int i = 0; i < sStatus.length; i++) {
                	/*
                  	rc = jado.InputProc("UP_KPI_O_S_기준실적처리",  v_sv_1[i]  + "," +
                                                                v_sv_2[i]  + "," +
                                                                v_sv_3[i]  + "," +
                                                                v_sv_4[i]  + "," +
                                                                v_sv_5[i]  + "," +
        			...
        			if(i==0) tmpQuery = jado.InputProcS("UP_KPI_O_S_기준실적처리", tmpParam);
                ...
                rc = jado.InputProcN(tmpQuery, tmpArrParam);
                  ...
                  //등록 종료시간
                  endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                  //web log 생성 : 기준실적 데이터 수가 많으므로 timeout 발생가능하므로 등록 여부만 log발생
                  jado.InputProc("UP_KPI_C_S_WEBLOG"
                                ,"5"                                 +","  //2:rpop,1:rpdy,5:운영관리
                                +"UP_KPI_O_S_기준실적처리"	           +","  //프로그램ID
                                +startTime                           +","  //시작시간
                                +endTime                             +","  //완료시간
                                +account.getHaengwon_no()            +","  //사용자ID
                                +account.getBranch_no()              +","  //점번호
