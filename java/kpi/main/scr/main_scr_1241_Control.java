...
/**
 *  클래스 개요     : 구조조정여신계좌 관리 <p>
 *  클래스 상세기능 : 구조조정여신계좌 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1241_Control extends HTMLActionSupport
{
      /**
       * HttpRequst의 정보를 구조조정여신계좌 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_구조조정여신계좌조회");
                ...
                rc = jado.InputProc("UP_KPI_O_S_구조조정여신계좌처리","4,0,0,0,0,0" );
               ...
               //구조조정여신계좌 삭제 종료시간
                ...
                String v_sv_1[]  = JSPUtil.getParameter(request,"v_sv_1"   ,0); //점번호
                ...
                for(int i = 0; i < sStatus.length; i++) {                	
                  rc = jado.InputProc("UP_KPI_O_S_구조조정여신계좌처리","1," +
                                                           v_sv_1[i]  + "," +
                                                           v_sv_2[i]  + "," +
                                                           v_sv_3[i]  + "," +
                                                           v_sv_4[i]  + "," +
