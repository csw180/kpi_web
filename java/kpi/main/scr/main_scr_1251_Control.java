...
/**
 *  클래스 개요     : 이수관계좌 관리 <p>
 *  클래스 상세기능 : 이수관계좌 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1251_Control extends HTMLActionSupport
{
      /**
       * HttpRequst의 정보를 이수관계좌 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_이수관계좌조회");
                ...
                rc = jado.InputProc("UP_KPI_O_S_이수관계좌처리","4,0,0,0,0,0,0,0,0" );
                ...
                String v_sv_1[]  = JSPUtil.getParameter(request,"v_sv_1"   ,0); //계좌관리점번호              
                String v_sv_2[]  = JSPUtil.getParameter(request,"v_sv_2"   ,0); //계좌번호
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"   ,0); //계정과목코드
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"   ,0); //고객번호
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"   ,0); //성명
                String v_sv_6[]  = JSPUtil.getParameter(request,"v_sv_6"   ,0); //이수관일
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"   ,0); //계좌수관점번호
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"   ,0); //수관점인정비율
                ...
                for(int i = 0; i < sStatus.length; i++) {                	
                  /*
                	rc = jado.InputProc("UP_KPI_O_S_이수관계좌처리","1," +
                                                           v_sv_1[i]  + "," +
                                                           v_sv_2[i]  + "," +                                                           
                                                           v_sv_3[i]  + "," +
                                                           v_sv_4[i]  + "," +                                                           
				...
				*/
                  ...
                  jado.InputProc("UP_KPI_C_S_WEBLOG"
                                ...
                                +account.getBranch_no()              +","  //점번호
                                +request.getRemoteAddr()             +","  //사용자IP
                                +"등록 : 계좌관리점번호=" +v_sv_1[i]  +";" //계좌관리점번호
                                +"계좌번호="              +v_sv_2[i]  +";" //계좌번호
                                +"계정과목코드="          +v_sv_3[i]  +";" //계정과목코드
                                +"계좌수관점번호="        +v_sv_7[i]  +";" //계좌수관점번호
                                +"계좌수관점인정비율="    +v_sv_8[i]  +";" //계좌수관점인정비율
                                );
					*/
                  ...
                  if(i==0) tmpQuery = jado.InputProcS("UP_KPI_O_S_이수관계좌처리", tmpParam);
