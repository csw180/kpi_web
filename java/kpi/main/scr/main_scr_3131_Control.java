...
/**
 *  클래스 개요    : Grid평점표 등록 <p>
 *  클래스 상세기능: Grid평점표 등록 JSP에 전달한다.<p>
 *  작성일 : 2011.02.14<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_3131_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 Grid평점표 조회 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String KpiCode    = JSPUtil.getParameter(request,"KpiCode"," ");
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_Grid평점표",basemonth + "," +
                                                         KpiCode);
                //조회종료시간
                ...
                jado.InputProc("UP_KPI_C_S_WEBLOG"
                              ...
                              +"KPI코드="         +KpiCode         + ";" //KPIcode
                              );
                                                         
            }else{ /***** 등록,변경,삭제 *****/
                rc = jado.InputProc("UP_KPI_O_S_Grid평점표삭제",basemonth + "," +
                                                             KpiCode);
                //삭제종료시간
                endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                ...
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_Grid평점표처리", v_sv_1[i]  + "," +
                                                                 v_sv_2[i]  + "," +
                                                                 v_sv_3[i]  + "," +
                                                                 v_sv_4[i]  + "," +
                                                                 strEmpNo
