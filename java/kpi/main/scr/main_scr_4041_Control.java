 ...
 *  @author  장보윤
 */
public class main_scr_4041_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 특수계좌 이익관리 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_특수계좌이익관리조회", basemonth);
               //조회종료시간
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_특수계좌이익관리삭제", basemonth);
                //삭제종료시간
                endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                ...
                String v_sv_12[] = JSPUtil.getParameter(request,"v_sv_12"   ,0);                              
                String v_sv_13[] = JSPUtil.getParameter(request,"v_sv_13"   ,0);   
                String v_sv_14[] = JSPUtil.getParameter(request,"v_sv_14"   ,0);   
                for(int i = 0; i < sStatus.length; i++) {
                	rc = jado.InputProc("UP_KPI_O_S_특수계좌이익관리등록",  v_sv_1[i]  + "," + 
                  	                                                    v_sv_2[i]  + "," + 
                  	                                                    v_sv_3[i]  + "," + 
                  	                                                    v_sv_4[i]  + "," + 
                  	                                                    v_sv_5[i]  + "," + 
                 ...
                 jado.InputProc("UP_KPI_C_S_WEBLOG"
                               ...
                               +"처리시작년월="    +v_sv_3[i]       + ";" //처리시작년월
                               +"처리종료년월="    +v_sv_4[i]       + ";" //처리종료년월
                               +"계좌번호="        +v_sv_5[i]       + ";" //계좌번호
                               +"계정과목코드="    +v_sv_6[i]       + ";" //계정과목코드
                               +"세목="            +v_sv_7[i]       + ";" //세목
                               +"수정전이익="      +v_sv_8[i]       + ";" //수정전이익
                               +"조정이익="        +v_sv_9[i]       + ";" //조정이익
                               +"수정후이익="      +v_sv_10[i]       + ";" //수정후이익
                               +"권유자번호="      +v_sv_11[i]       + ";" //권유자번호
                               +"PI이익구분="      +v_sv_12[i]      + ";" //PI이익구분
                               +"평가대상구분="    +v_sv_13[i]      + ";" //평가대상구분
                               +"반영구분="        +v_sv_14[i]      + ";" //반영구분
                               );
