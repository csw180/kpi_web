...
/**
 *  클래스 개요    : 가중평균연체비율 조회  <p>
 *  클래스 상세기능 : 가중평균연체비율을 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2006.07.01<p>
 *  @version 1.0
 *  @author  변휘원
 */
public class rpop_scr_2030_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String pdtgubun   = JSPUtil.getParameter(request, "pdtgubun"," "); 
        ...
        try
        {
            ...
            //프로시져명 설정
            if (baseMon <= 200812) procName = "UP_KPI_M_S_가중평균연체비율2008";
            else  procName = "UP_KPI_M_S_가중평균연체비율" + basemonth.substring(0,4);

            //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();
            rs = jado.callProc(procName, basemonth +","+ v_jumcode+","+pdtgubun+","+bgubun+","+mgubun );
