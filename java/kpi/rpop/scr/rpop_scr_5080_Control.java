...
/**
 *  클래스 개요     : 후선보임 세부실적 조회  <p>
 *  클래스 상세기능 : 후선보임 세부실적 보고서를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2013.11.20<p>
 *  @version 1.0
 *  @author  조형석
 */
public class rpop_scr_5080_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 후선보임 세부실적 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String jikwonno   = JSPUtil.getParameter(request, "jikwonno"," ");
        String stmonth    = JSPUtil.getParameter(request, "stmonth"," ");
        String edmonth    = JSPUtil.getParameter(request, "edmonth"," ");
        ...
        try
        {
            ...
            rs = jado.callProc("UP_KPI_M_S_후선보임세부실적", jikwonno +","+ stmonth+ "," + edmonth);
