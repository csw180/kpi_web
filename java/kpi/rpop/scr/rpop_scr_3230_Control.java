...
/**
 *  클래스 개요     : 고객수증대 세부내역 조회 <p>
 *  클래스 상세기능 : 고객수증대 세부내역을 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2022.03.07<p>
 *  @version 1.0
 *  @author  양인찬
 */
public class rpop_scr_3230_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            //프로시져명 설정
            procName = "UP_KPI_M_S_고객수증대세부내역" + basemonth.substring(0,4);
            
            //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();

            rs = jado.callProc(procName, basemonth+","+v_jumcode);
