  ...
  public static ArrayList getExceptionMappings(Element root) {
    ...
    for (int loop=0; loop<list.getLength(); loop++) {
      ...
      if (node!=null) {
        ...
        String screen=null;
        // get exception nodes
        // need to be chb.framework.component.error element to get attributes
  ...
  public static HashMap getEventMappings(Element root) {
    ...
    for (int loop=0; loop<list.getLength(); loop++) {
      ...
      if (node!=null) {
        ...
        //log.debug("eventClassName = "+eventClassName+" commandClassName= "+commandClassName);
    ...
    for (int loop=0; loop<list.getLength(); loop++) {
      ...
      if (node!=null) {
        ...
        if ((useFlowHandlerString!=null)&&useFlowHandlerString.equals("true"))
          useFlowHandler=true;
        if (useFlowHandler) {
          // need to be chb.framework.component.error element to find sub nodes by name
          if (node instanceof Element) {
            Element element=(Element)node;
            ...
            if (children.getLength()>1) {
              System.err.println("Non fatal error: There can be only one <"+FLOW_HANDLER+
                                 "> definition in chb.framework.component.error <"+URL_MAPPING+">");
            }
