...
public class TemplateServlet extends HttpServlet {
	...
	private void initScreens(ServletContext context, String language) {
		...
		if (screenDefinitionURL != null) {
			...
			if (screenDefinitions != null) {
				...
				System.err.println(
						"Template Servlet Error Loading Screen Definitions: Confirm that file at URL /WEB-INF/screendefinitions_"
								+ language + ".xml contains the screen definitions");
			}
		} else {
			System.err.println("Template Servlet Error Loading Screen Definitions: URL /WEB-INF/screendefinitions_"
	...
	public void process(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		// The language when specified as chb.framework.component.error parameter
		// overrides the language set in the session
		...
		if (screenName.equals(PREVIOUS_SCREEN)) {
			...
			if (cachePreviousScreenParameters) {
				// copy all the parameters into chb.framework.component.error new map
			...
			if (cachePreviousScreenAttributes) {
				// put the request attributes into chb.framework.component.error map
		...
		if (screenName != null) {
			...
			// default to the default locale screen if it was not defined in the locale
	...
	private void insertTemplate(HttpServletRequest request, HttpServletResponse response, String templateName)
			throws IOException, ServletException {

		// This method tries to wrap the request dispatcher call with-in
		// chb.framework.component.error transaction for making the local EJB access
		// efficient. However,
		// it is not chb.framework.component.error fatal error, if such
		// chb.framework.component.error transaction can not be started
		...
		try {
			...
			// it should not have happened, but it is chb.framework.component.error
			// recoverable error.
			// Just dont start the transaction.
			// ne.printStackTrace();
			log.error("err", ne);
		} catch (NotSupportedException nse) {
			// Again this is chb.framework.component.error recoverable error.
			// nse.printStackTrace();
			log.error("err", nse);
