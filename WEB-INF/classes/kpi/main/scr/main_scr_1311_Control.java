...
/**
 *  클래스 개요    : 외환실적 조정 등록 <p>
 *  클래스 상세기능: 외환실적 조정 등록 JSP에 전달한다.<p>
 *  작성일 : 2012.02.17<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1311_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 외환실적 조정 등록 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String sjgubun    = JSPUtil.getParameter(request,"sjgubun"," ");
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_FX실적조정",basemonth + "," +
                                                        sjgubun);
                //조회종료시간
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                             ...
                             +"실적구분(1:coporate deal)="        +sjgubun         + ";" //실적구분
                             );

            }else{ /***** 등록,변경,삭제 *****/
                rc = jado.InputProc("UP_KPI_O_S_FX실적조정삭제",basemonth + "," +
                                                              sjgubun);
                //삭제 종료시간
                endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                //web log 생성 
                ...
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_FX실적조정처리", v_sv_1[i]  + "," +
                                                                 v_sv_2[i]  + "," +
                                                                 v_sv_3[i]  + "," +
                                                                 v_sv_4[i]  + "," +
                                                                strEmpNo
                  ...
                  jado.InputProc("UP_KPI_C_S_WEBLOG"
                                ...
                                +"점번호="                           +v_sv_2[i]       + ";" //점번호
                                +"실적구분(1:coporate deal)="        +v_sv_3[i]       + ";" //실적구분
                                +"조정실적="                         +v_sv_4[i]       + ";" //조정실적
                                );
