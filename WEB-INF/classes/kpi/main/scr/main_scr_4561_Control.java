...
/**
 *  클래스 개요     : 부지점장 기준실적 조회 및 처리  <p>
 *  클래스 상세기능 : 부지점장 기준실적 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2015.03.<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_4561_Control extends HTMLActionSupport   
{
    /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_부지점장기준실적조회",basemonth);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_부지점장기준실적삭제",basemonth);
                ...
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"    ,0);  //사업부소속코드
                String v_sv_6[]  = JSPUtil.getParameter(request,"v_sv_6"    ,0);  //기준실적상품분류코드
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"    ,0);  //반기1st월기준실적
                String v_sv_9[]  = JSPUtil.getParameter(request,"v_sv_9"    ,0);  //반기2nd월기준실적
                String v_sv_10[] = JSPUtil.getParameter(request,"v_sv_10"   ,0);  //반기3rd월기준실적
                String v_sv_11[] = JSPUtil.getParameter(request,"v_sv_11"   ,0);  //반기4th월기준실적
                String v_sv_12[] = JSPUtil.getParameter(request,"v_sv_12"   ,0);  //반기5th월기준실적
                String v_sv_13[] = JSPUtil.getParameter(request,"v_sv_13"   ,0);  //반기6th월기준실적
                String  strEmpNo = account.getHaengwon_no();

                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_부지점장기준실적처리",  v_sv_1[i]  + "," +
                                                                        v_sv_2[i]  + "," +
                                                                        v_sv_3[i]  + "," +
                                                                        v_sv_4[i]  + "," +
                                                                        v_sv_5[i]  + "," +
