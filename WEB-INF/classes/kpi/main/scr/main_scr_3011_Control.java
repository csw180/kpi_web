...
/**
 *  클래스 개요     : 영업점목표등록  <p>
 *  클래스 상세기능 : 영업점목표 등록 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2006.07.01<p>
 *  @version 1.0
 *  @author  변휘원
 */
public class main_scr_3011_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String v_jumcode  = JSPUtil.getParameter(request,"v_jumcode","0");
        ...
        try
        {
            ...
            if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_영업점목표",basemonth+","+v_jumcode);
                //조회종료시간
                ...
                if (hidden_key.equals("1")){
                    rc = jado.InputProc("UP_KPI_O_S_영업점목표삭제", basemonth);
                    //삭제종료시간
                    endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                ...
                String v_sv_2[]  = JSPUtil.getParameter(request,"v_sv_2"    ,0);   //kpi코드
                String v_sv_3[]  = JSPUtil.getParameter(request,"v_sv_3"    ,0);   //점번호
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"    ,0);   //사업부소속코드
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"    ,0);   //기준실적상품분류코드
                String v_sv_6[]  = JSPUtil.getParameter(request,"v_sv_6"    ,0);   //1분기
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"    ,0);   //2분기
                String v_sv_8[]  = JSPUtil.getParameter(request,"v_sv_8"    ,0);   //3분기
                String v_sv_9[]  = JSPUtil.getParameter(request,"v_sv_9"    ,0);   //4분기
                String v_sv_10[] = JSPUtil.getParameter(request,"v_sv_10"   ,0);   //1st월
                String v_sv_11[] = JSPUtil.getParameter(request,"v_sv_11"   ,0);   //2nd월
                String v_sv_12[] = JSPUtil.getParameter(request,"v_sv_12"   ,0);   //3rd월
                String v_sv_13[] = JSPUtil.getParameter(request,"v_sv_13"   ,0);   //4th월
                String v_sv_14[] = JSPUtil.getParameter(request,"v_sv_14"   ,0);   //5th월
                String v_sv_15[] = JSPUtil.getParameter(request,"v_sv_15"   ,0);   //6th월                
                ...
                for(int i = 0; i < sStatus.length; i++) {
                	...
                	/*
                	rc = jado.InputProc("UP_KPI_O_S_영업점목표처리", hidden_key  + "," +
                                                                v_sv_1[i]   + "," + 
                                                                v_sv_2[i]   + "," + 
                                                                v_sv_3[i]   + "," + 
                                                                v_sv_4[i]   + "," + 
        			...
        			if(i==0) tmpQuery = jado.InputProcS("UP_KPI_O_S_영업점목표처리", tmpParam);
