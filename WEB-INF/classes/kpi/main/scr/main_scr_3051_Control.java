...
/**
 *  클래스 개요    : 영업점/RM(본부)/RM(영업점) 목표 등록 이력 조회  <p>
 *  클래스 상세기능 : 영업점/RM(본부)/RM(영업점) 목표 등록 이력 내역을 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2021.06.01<p>
 *  @version 1.0
 *  @author  양인찬
 */
public class main_scr_3051_Control extends HTMLActionSupport   
{
      /**
       * HttpRequst의 정보를 목표 등록 이력 조회 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_목표등록이력조회",basemonth+","+v_jumcode);
                //조회종료시간
