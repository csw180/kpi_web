...
/**
 *  클래스 개요     : 부지점장 실적조정계좌  <p>
 *  클래스 상세기능 : 부지점장 실적조정계좌 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2015.03.19<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_4581_Control extends HTMLActionSupport   
{
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_부지점장실적조정계좌조회",basemonth+","+jumcode);
                //조회종료시간
                ...
                if (hidden_key.equals("1")){
                    rc = jado.InputProc("UP_KPI_O_S_부지점장실적조정계좌삭제", basemonth);
                    //삭제종료시간
                    endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                    //web log 생성 
                    jado.InputProc("UP_KPI_C_S_WEBLOG"
                                  ,"5"                                 +","  //2:rpop,1:rpdy,5:운영관리
                                  +"부지점장 실적조정계좌 삭제"        +","  //프로그램ID
                                  +startTime                           +","  //시작시간
                                  +endTime                             +","  //완료시간
                                  +account.getHaengwon_no()            +","  //사용자ID
                                  +account.getBranch_no()              +","  //점번호
