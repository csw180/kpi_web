...
/**
 *  클래스 개요    : 사업부 변경 고객 조회 및 처리  <p>
 *  클래스 상세기능 : 사업부 변경 고객 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1121_Control extends HTMLActionSupport
{
      /**
       * HttpRequst의 정보를 사업부 변경 고객 조회 데이타모델로 파싱하여 반환한다.
       */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               ...
               rs = jado.callProc("UP_KPI_O_S_사업부변경고객",basemonth);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_사업부변경고객삭제",basemonth);
                ...
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_사업부변경고객처리", v_sv_1[i]  + "," +
                                                                     v_sv_2[i]  + "," +
                                                                     v_sv_3[i]  + "," +
                                                                     v_sv_4[i]  + "," +
                                                                     v_sv_5[i]  + "," +
