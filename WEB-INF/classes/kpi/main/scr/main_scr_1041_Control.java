...
/**
 *  클래스 개요     : 타지점실적계좌 승인  <p>
 *  클래스 상세기능 : 타지점실적등록 계좌를 성과담당자가 승인한다.<p>
 *  작성일 : 2006.07.01<p>
 *  @version 1.0
 *  @author  변휘원
 */
public class main_scr_1041_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                ...
                //타지점실적계좌 승인  시작시간
                startTime = tpms.framework.component.util.DateTime.getShortTimeString();                	
                rc = jado.InputProc("UP_KPI_O_S_타지점실적계좌승인", v_sv_1[i]  + "," + //계좌관리점번호
                                                                   v_sv_3[i]  + "," + //계정과목코드
                                                                   v_sv_5[i]  + "," + //계좌번호
                                                                   v_sv_13[i] + "," + //승인여부
                                                                   v_sv_11[i] + "," + //등록부서구분
        ...
        { 
            ...
            // Error message 생성 --------------------------------------------->
            if (hidden_key.equals("9")) { /***** 조회 *****/
                message = "타지점실적계좌 조회 정보를 확인하세요";
            }else   /***** 승인 *****/
                message = "타지점실적계좌 승인 정보를 확인하세요";
            // ----------------------------------------------------------------/            
            ext    =    exe;
        }
/****************************변경************************/
