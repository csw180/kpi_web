...
/**
 *  클래스 개요     : 자본비용 조회 및 처리  <p>
 *  클래스 상세기능 : 자본비용 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2011.01.<p>
 *  @version 1.0
 *  @author  하진영
 */
public class main_scr_1221_Control extends HTMLActionSupport   
{
    /**
     * HttpRequst의 정보를 자본비용 조회 데이타모델로 파싱하여 반환한다.
     */
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
               rs = jado.callProc("UP_KPI_O_S_자본비용",basemonth+","+v_jumcode);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_자본비용삭제",basemonth);
                ...
                for(int i = 0; i < sStatus.length; i++) {
                  rc = jado.InputProc("UP_KPI_O_S_자본비용처리",   v_sv_1[i]  + "," +
                                                                 v_sv_2[i]  + "," +
                                                                 v_sv_3[i]  + "," +
                                                                 v_sv_4[i]  + "," +
                                                                 strEmpNo
