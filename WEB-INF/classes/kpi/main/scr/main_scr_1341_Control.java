...
/**
 *  클래스 개요     : 본부등록 실적 관리 조회 및 처리  <p>
 *  클래스 상세기능 : 본부등록 실적 관리 upload 처리 및 조회하여 JSP에 전달한다.<p>
 *  작성일 : 2015.07.28<p>
 *  @version 1.0
 *  @author  조형석
 */
public class main_scr_1341_Control extends HTMLActionSupport   
{
    /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
           ...
           if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_본부등록실적조회",baseday+","+kpicode);
               ...
               jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                rc = jado.InputProc("UP_KPI_O_S_본부등록실적처리","1,"+baseday+","+kpicode+",0,0,0");
                ...
                for(int i = 0; i < sStatus.length; i++) {
                    rc = jado.InputProc("UP_KPI_O_S_본부등록실적처리","2,"+ v_sv_1[i]  + "," +
                                                                          kpicode    + "," +
                                                                          v_sv_2[i]  + "," +
                                                                          v_sv_3[i]  + "," +
                                                                          strEmpNo
