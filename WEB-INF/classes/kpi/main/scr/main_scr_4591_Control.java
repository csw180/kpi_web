...
/**
 *  클래스 개요     : 부지점장지정업무  <p>
 *  클래스 상세기능 : 부지점장지정업무 정보를 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2021.08.19<p>
 *  @version 1.0
 *  @author  양인찬
 */
public class main_scr_4591_Control extends HTMLActionSupport   
{
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            if (hidden_key.equals("9")){  /***** 조회 *****/
                rs = jado.callProc("UP_KPI_O_S_부지점장지정업무조회",basemonth+","+jumcode);
                //조회종료시간
                ...
                if (hidden_key.equals("1")){
                    rc = jado.InputProc("UP_KPI_O_S_부지점장지정업무삭제", basemonth);
                    //삭제종료시간
                    endTime = tpms.framework.component.util.DateTime.getShortTimeString();
                    //web log 생성 
                    jado.InputProc("UP_KPI_C_S_WEBLOG"
                ...
                String v_sv_4[]  = JSPUtil.getParameter(request,"v_sv_4"    ,0);   //부지점장KPI코드
                String v_sv_5[]  = JSPUtil.getParameter(request,"v_sv_5"    ,0);   //부지점장KPI명
                String v_sv_6[]  = JSPUtil.getParameter(request,"v_sv_6"    ,0);   //지정관리업무
                String v_sv_7[]  = JSPUtil.getParameter(request,"v_sv_7"    ,0);   //지정관리업무명
                String strEmpNo     = account.getHaengwon_no();                    //직원번호
                for(int i = 0; i < sStatus.length; i++) {
                rc = jado.InputProc("UP_KPI_O_S_부지점장지정업무처리", hidden_key + "," +
                                                                     v_sv_1[i]   + "," + 
                                                                     v_sv_2[i]   + "," + 
                                                                     v_sv_3[i]   + "," + 
                                                                     v_sv_4[i]   + "," + 
