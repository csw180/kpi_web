...
/**
 *  클래스 개요    : 영업점 총예수금 평가내역 조회  <p>
 *  클래스 상세기능 : 영업점 총예수금 평가내역을 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2008.04.24<p>
 *  @version 1.0
 *  @author  조형석
 */
public class rpop_scr_5040_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        String saupgbn    = JSPUtil.getParameter(request, "saupgbn"," ");
        String pgcode     = JSPUtil.getParameter(request, "pgcode"," ");
        String sergb      = JSPUtil.getParameter(request, "sergb"," ");
        String WideCode   = JSPUtil.getParameter(request, "WideCode"," ");
        ...
        try
        {
            ...
            //프로시져명 설정            
            if (baseMon <= 201012) procName = "UP_KPI_M_S_총예수금평가내역2010";                
                else  procName = "UP_KPI_M_S_총예수금평가내역"+ basemonth.substring(0,4);          
           
           //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();

            rs = jado.callProc(procName,basemonth+","+saupgbn+","+pgcode+","+sergb+","+WideCode);
