...
/**
 *  클래스 개요     : RM 예수금 내역 조회  <p>
 *  클래스 상세기능 : RM 예수금 내역을 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2012.03.19<p>
 *  @version 1.0
 *  @author  하진영
 */
public class rpop_scr_5120_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            //프로시져명 설정
            if (baseMon <= 201806) procName = "UP_KPI_M_S_RM예수금내역";
            else  procName = "UP_KPI_M_S_RM예수금내역" + basemonth.substring(0,4);
            
            //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();

            rs = jado.callProc(procName, basemonth+","+pgcode+","+sergb);
