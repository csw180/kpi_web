...
/**
 *  클래스 개요     : 부지점장총괄세부실적 조회  <p>
 *  클래스 상세기능 : 부지점장총괄세부실적 읽어서 JSP에 전달한다.<p>
 *  작성일 : 2014.07.18<p>
 *  @version 1.0
 *  @author  조형석
 */
public class rpop_scr_7230_Control extends HTMLActionSupport   
{
      /**
    ...
    public EventResponse perform(HttpServletRequest request) throws HTMLActionException {
        ...
        try
        {
            ...
            //프로시져명 설정
            procName = "UP_KPI_M_S_부지점장총괄세부실적" + basemonth.substring(0,4);
            
            //조회시작시간
            String startTime = tpms.framework.component.util.DateTime.getShortTimeString();

            rs = jado.callProc(procName, basemonth+","+period);
