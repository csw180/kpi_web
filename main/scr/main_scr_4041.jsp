...
'프로그램 ID  : main_scr_4041.jsp
'프로그램 명  : 특수계좌 이익관리
'프로그램개요 : 특수계좌 이익관리
'작   성   자 : 장보윤
'작   성   일 : 2012.06.
====================================================================
'수정자/수정일/수정사유 :하진영/2015.01.16/특정이익 입력기간동안 인정
===============================================================--%>
...
%>
    <TR>
      ...
      <TD><![CDATA[<%=rs.getString("평가반영년월").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("처리시작년월").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("처리종료년월").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("계좌번호").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("계정과목코드").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("계정세과목코드").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("개인PI수정전이익").trim()%>]]></TD>
      <TD><![CDATA[<%=rs.getString("개인PI조정이익").trim()%>]]></TD>      
      <TD><![CDATA[<%=rs.getString("개인PI반영이익").trim()%>]]></TD>            
      <TD><![CDATA[<%=rs.getString("권유자직원번호").trim()%>]]></TD>            
      <TD><![CDATA[<%=rs.getString("개인PI이익구분").trim()%>]]></TD>                        
      <TD><![CDATA[<%=rs.getString("평가대상구분").trim()%>]]></TD>       
      <TD><![CDATA[<%=rs.getString("개인PI추가여부").trim()%>]]></TD>          
    </TR> 
<%
            } /* end while */
            strMsg = "조회 완료 되었습니다.";
