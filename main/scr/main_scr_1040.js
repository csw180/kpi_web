...
function LoadPage(){
	...
	mySheet.SetConfig( { SearchMode:2, MergeSheet:0, Page:20, FrozenCol:0 } );
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"계좌관리점|계좌관리점명|계정과목코드|계정과목명|계좌번호|고객명|신규일|성과관리점|성과관리점명|성과관리점\n인정비율(%)|등록부서|등록일|승인여부|상태", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_1",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Left",    SaveName:"v_sv_9",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Float",     Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_10",   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
             {Type:"Combo",     Hidden:0, MinWidth:100,  Align:"Right",   SaveName:"v_sv_11",   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:1,   InsertEdit:0,   EditLen:2 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_12",   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"CheckBox",  Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_sv_13",   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:1,   InsertEdit:0 },
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "고정필드해제":
        	...
        	break;
        case "조회":             // 조회(상위 GRID)
            mySheet.DoSearch("main.scr.main_scr_1041.do?hidden_key=9"); 
            break;
        ...
        case "등록":            // 등록
            ...
            mySheet.DoSave( "main.scr.main_scr_1041.do?hidden_key=1");
            break;
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=타지점예금계좌승인&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=타지점예금계좌승인&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=타지점예금계좌승인&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
