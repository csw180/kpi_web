...
function LoadPage()
{
    ...
    var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
    var headers = [ { Text:"상태|작업기준년월|점번호|KPI코드|평가구분|조정실적|비고", Align:"Center"} ];
	...
	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Combo",     Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_sv_4",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Float",     Hidden:0,  MinWidth:130,  Align:"Right",   SaveName:"v_sv_5",    CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   InsertEdit:0,   EditLen:12 },
             {Type:"Text",      Hidden:0,  MinWidth:200,  Align:"Left",    SaveName:"v_sv_6",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:1,   InsertEdit:1,   EditLen:30 } ];
       
    mySheet.InitColumns(cols);

	mySheet.SetEditable(1);
	mySheet.SetVisible(1);
	mySheet.SetColProperty(4, {ComboText:"월|분기|반기|연간", ComboCode:"1|2|3|4"} );
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                    // 조회
            mySheet.DoSearch("main.scr.main_scr_1141.do?hidden_key=9&basemonth=" + basemonth); 
			showUnitText("원");
            break;
...
/*                if(mySheet.CellValue(i, 3) < 31 || mySheet.CellValue(i, 3) > 48 ) 
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 영업점조정실적 데이터는 삭제되고, \n\n" + 
             ...
             var isSuccess = mySheet.DoSave("main.scr.main_scr_1141.do", {Param : "hidden_key=2&basemonth=" + basemonth});
             if(!isSuccess) return;
...
function selectcommit(chk)
{
    ...
    // 확정구분 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=영업점실적조정등록&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=영업점실적조정등록&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=영업점실적조정등록&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
