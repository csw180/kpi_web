...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                 // 조회
             mySheet.DoSearch("main.scr.main_scr_4531.do?hidden_key=9&basemonth="+basemonth+"&jumcode="+jumcode); 
             break;
        case "엑셀내려받기":            // 엑셀내려받기
             inqText       = new Array();
             inqText[0]    = new Array();
...
*/
        ...
        case "저장":                    // 저장
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 RM(영업점)배점 데이터는 삭제되고, \n\n" + 
                                              "Excel 업로드된 데이터로 반영되게됩니다.\n\n등록하시겠습니까?"))
             {
                return;
             }
             
             /*var isSuccess = mySheet.DoSave("main.scr.main_scr_4531.do?hidden_key=1&basemonth=" + basemonth, "", "", false);*/ 
             var isSuccess = mySheet.DoSave("main.scr.main_scr_4531.do", {Param : "hidden_key=1&basemonth=" + basemonth}); 
             if(!isSuccess) return;
             break;
    }
}
...
function setGrid2014()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|직원번호|점번호|KPI코드|배점", Align:"Center"} ];
