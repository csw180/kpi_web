...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)          
             hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&v_inqtype=1&jekyocode=4&stmonth=201801");                 
            break;
        ...
        case "조회":                 // 조회
             if(sjgubun == 99)
            {
             ...
             if(basemonth < "201801" )
            {
              alert("수산해양업종대출관리 는 2018년부터 조회 가능합니다.");
              return;
            } 
        ...
        case "저장":                    // 저장
             ...
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_1361.do?hidden_key=2&basemonth=" + basemonth + "&sjgubun=" + sjgubun, "", "", false);
             var isSuccess = mySheet.DoSave("main.scr.main_scr_1361.do", {Param : "hidden_key=2&basemonth=" + basemonth + "&sjgubun=" + sjgubun});
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=수산해양업종대출관리&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=수산해양업종대출관리&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=수산해양업종대출관리&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
...
{   
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|점번호|계좌번호|조정실적", Align:"Center"} ];
