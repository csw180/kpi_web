...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&v_inqtype=1&jekyocode=4&stmonth=201601"); 
            break;
        case "조회":                 // 조회
             mySheet.DoSearch("main.scr.main_scr_4591.do?hidden_key=9&basemonth="+basemonth); 
             break;
        case "엑셀내려받기":            // 엑셀내려받기
             inqText       = new Array();
             inqText[0]    = new Array();
...
*/
        ...
        case "초기화":                  // 초기화
             ...
             break;   
        case "저장":                    // 저장
             var Row = mySheet.ColValueDup("1|2|3|4|6");
             if(Row>-1)
             {
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 RM(영업점) 지정업무 데이터는 삭제되고, \n\n" + 
                                              "Excel 업로드된 데이터로 반영되게됩니다.\n\n등록하시겠습니까?"))
             {
                return;
             }
             
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_4591.do?hidden_key=1&basemonth=" + basemonth, "", "", false); 
             var isSuccess = mySheet.DoSave("main.scr.main_scr_4591.do", {Param : "hidden_key=1&basemonth=" + basemonth}); 
             if(!isSuccess) return;
             break;
    }
}
...
function setGrid2021()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|직원번호|점번호|부지점장KPI코드|부지점장KPI명|지정관리업무|지정관리업무명", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_5",    CalcLogic:"",   Format:"",            PointCount:2,   UpdateEdit:0,   InsertEdit:0,   EditLen:30 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Left",   SaveName:"v_sv_7",    CalcLogic:"",   Format:"",            PointCount:2,   UpdateEdit:0,   InsertEdit:0,   EditLen:30 } ];
