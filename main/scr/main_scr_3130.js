...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "조회":                    // 조회
             if(KpiCode == 99)
            {
              alert("KPI코드 선택후 조회하세요");
              return;
            } 

             if(basemonth < "201101" )
            {
              alert("GRID평점표는 2011년이후부터 등록/조회 가능합니다.");
              return;
            } 
             mySheet.DoSearch("main.scr.main_scr_3131.do?hidden_key=9&basemonth=" + basemonth + "&KpiCode=" + KpiCode); 
             isEnableSave        = true;
...
*/                          
        ...
        case "저장":                    // 저장
             ...
             {
                if(mySheet.CellValue(i, 1)!=basemonth)
                {
...
/*                if(mySheet.CellValue(i, 3) < 31 || mySheet.CellValue(i, 3) > 48 ) 
             ...
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_3131.do?hidden_key=2&basemonth=" + basemonth + "&KpiCode=" + KpiCode, "", "", false);
             var isSuccess = mySheet.DoSave("main.scr.main_scr_3131.do", {Param : "hidden_key=2&basemonth=" + basemonth + "&KpiCode=" + KpiCode });
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=Grid평점표등록&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=Grid평점표등록&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=Grid평점표등록&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
...
function setKpiCode(){
	...
	var KpiCodeArr = new Array();
    
	if( basemonth >= '201301'){
		KpiCodeArr[0] = new Array('99' , '선택');
    	KpiCodeArr[1] = new Array('247', '연체관리');
    	KpiCodeArr[2] = new Array('248', 'RM(영업점) 연체관리');
	}
	else {
		KpiCodeArr[0]  = new Array('99' , '선택');
		KpiCodeArr[1]  = new Array('113', '신용카드연체비율(현상)');
		KpiCodeArr[2]  = new Array('136', '대출연체비율(현상)');
		KpiCodeArr[3]  = new Array('160', '여신포트폴리오비율');
		KpiCodeArr[4]  = new Array('161', '여신포트폴리오비율(개선도)');
		KpiCodeArr[5]  = new Array('205', '대출금연체비율(개선도)');
		KpiCodeArr[6]  = new Array('206', '신용카드연체비율(개선도)');
		KpiCodeArr[7]  = new Array('217', '신규발생고정이하여신비율');
    	KpiCodeArr[8]  = new Array('232', '[상반기]신용카드연체비율(현상)');
    	KpiCodeArr[9]  = new Array('231', '[상반기]대출연체비율(현상)');
    	KpiCodeArr[10] = new Array('233', '[상반기]신규발생고정이하여신비율');
	}
	setCombo(KpiCodeArr);			
}

function setCombo(KpiCodeArr){
	el = document.frm.KpiCode;	
	while(el.options.length > 0){el.options.remove(0);}

  	for(i=0;i<parseInt(KpiCodeArr.length);i++){	   
    	var oOption = document.createElement("OPTION");
	  	oOption.value = KpiCodeArr[i][0];
    	oOption.text  = KpiCodeArr[i][1];       
    	el.options.add(oOption);
  	}
}
...
function initGrid()
{   
	  v_KpiCode = document.frm.KpiCode.value; 
   	    
   	//대출금연체비율,신용카드연체비율,(상반기)대출금연체비율,(상반기)신용카드연체비율,연체관리    
    if (v_KpiCode == '136'|| v_KpiCode == '113' || v_KpiCode == '231'|| v_KpiCode == '232' )  setGridOverdue();  
    else if (v_KpiCode == '205' || v_KpiCode == '206')  setGridImprovement();               //대출금연체비율개선도, 신용카드연체비율개선도
         else if (v_KpiCode == '160')  setGridLoanPotpol();           //여신포트폴리오비율
          	    else if (v_KpiCode == '161')  setGridImprovement();  //여신포트폴리오비율개선도
           	    	   else if (v_KpiCode == '217' || v_KpiCode == '233')  setGridNewFix();      //신규발생고정이하비율,(상반기)신규발생고정이하비율
           	    	   	    else if ( v_KpiCode == '247' || v_KpiCode == '248' ) setGridOverdueRate()   // 연체관리 2013년 추가 평점인정비율을 가짐.
    
}

/*--------------------------------------------------------------*/
/* @기능 : KPI코드(연체관리)에 따른 Grid set                    */
/*       : 점별 배점이 달라 평점인정비율을 가지고 점수산출해야함*/
/*--------------------------------------------------------------*/   
function setGridOverdueRate()
{
  	//mySheet.RemoveAll();
	mySheet.Reset();
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|KPI코드|연체비율|평점인정비율(%)", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Float",     Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_3",    KeyField:1,   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   InsertEdit:0,   EditLen:6 },
             {Type:"Float",     Hidden:0,  MinWidth:130,  Align:"Right",   SaveName:"v_sv_4",    CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   InsertEdit:0,   EditLen:6 } ];
...
/*--------------------------------------------------------------*/
/* @기능 : KPI코드(신규발생고정이하비율)에 따른 Grid set     */
/*--------------------------------------------------------------*/   
function setGridNewFix()
{
  	//mySheet.RemoveAll();
  	mySheet.Reset();
	...
	mySheet.SetConfig( { SearchMode:2, MergeSheet:0, Page:20, FrozenCol:0 } );
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|KPI코드|부실비율|GRID평점", Align:"Center"} ];
   	mySheet.InitHeaders(headers, info);
...
/*--------------------------------------------------------------*/
/* @기능 : KPI코드(대출금,신용카드연체비율)에 따른 Grid set     */
/*--------------------------------------------------------------*/   
function setGridOverdue()
{
  	//mySheet.RemoveAll();
  	mySheet.Reset();
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|KPI코드|연체비율|GRID평점", Align:"Center"} ];
   	mySheet.InitHeaders(headers, info);
...
/*------------------------------------------------------*/
/* @기능 : KPI코드(연체비율 개선도)에 따른 Grid set     */
/*------------------------------------------------------*/   
function setGridImprovement()
{
  	//mySheet.RemoveAll();
  	mySheet.Reset();
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|KPI코드|개선도|평점", Align:"Center"} ];
   	mySheet.InitHeaders(headers, info);
...
/*------------------------------------------------------*/
/* @기능 : KPI코드(여신포트폴리오비율)에 따른 Grid set  */
/*------------------------------------------------------*/   
function setGridLoanPotpol()
{
  	//mySheet.RemoveAll();
  	mySheet.Reset();
	...
	// 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|KPI코드|여신포트폴리오비율|평점", Align:"Center"} ];
   	mySheet.InitHeaders(headers, info);
