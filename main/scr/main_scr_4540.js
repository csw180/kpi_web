...
function GridSizeByIsPopup() {
	if (getUrlParameter ('IS_POPUP') == "Y") {
  		createIBSheet2(  document.getElementById("sheetObj"),"mySheet", "100%", "450px");
	} else {
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "조회":                 // 조회
            mySheet.DoSearch("main.scr.main_scr_4541.do?hidden_key=9&basemonth="+basemonth+"&jumcode="+jumcode+"&pygb="+pygb); 
			showUnitText("원");
            break;
...
*/			
        ...
        case "저장":                    // 저장
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 기타조정손익 데이터는 삭제되고, \n\n" + 
                                              "Excel 업로드된 데이터로 반영되게됩니다.\n\n등록하시겠습니까?"))
             {
                return;
             }
             
//             var isSuccess = mySheet.DoSave("main.scr.main_scr_4541.do?hidden_key=1&basemonth=" + basemonth, "", "", false); 
             var isSuccess = mySheet.DoSave("main.scr.main_scr_4541.do", {Param : "hidden_key=1&basemonth=" + basemonth}); 
             if(!isSuccess) return;
             break;
    }
}
...
function setGrid2014()
{
	...
	mySheet.SetConfig( { SearchMode:2, MergeSheet:0, Page:20, FrozenCol:0 } );
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|직원번호|점번호|조정구분|평가구분|RM(영업점)고객구분|조정금액|조정구분명", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_4",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:3 },
             {Type:"Combo",     Hidden:0, MinWidth:100,  Align:"Center",   SaveName:"v_sv_5",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:1 },
             {Type:"Combo",     Hidden:0, MinWidth:100,  Align:"Center",   SaveName:"v_sv_6",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:1 },
             {Type:"Int",       Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_7",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:1,   InsertEdit:0,   EditLen:12 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_8",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 } ];
   	
   	mySheet.InitColumns(cols);
	mySheet.SetColProperty(5, {ComboText:"월|분기|대손후|대손전", ComboCode:"1|2|3|4"} );
   	mySheet.SetColProperty(6, {ComboText:"신규고객|지정고객", ComboCode:"1|2"} );    
