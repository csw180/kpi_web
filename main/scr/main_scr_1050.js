...
function LoadPage()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|점번호|조정구분|평가구분|조정금액", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Combo",     Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_sv_4",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:1 },
             {Type:"Int",       Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_5",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:12 } ];
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                    // 조회
             mySheet.DoSearch("main.scr.main_scr_1051.do?hidden_key=9&basemonth=" + basemonth); 
             break;
...
*/
        ...
        case "저장":                    // 저장
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 기타조정손익항목 데이터는 삭제되고, \n\n" + 
                                              "Excel 업로드된 데이터로 반영되게됩니다.\n\n등록하시겠습니까?"))
             {
                return;
             }
             
             //20220210 한종보 대상작업 미진행 코드
			 //concatPrimaryKeys(mySheet, "1|2|3|4|5");
             
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_1051.do?hidden_key=2&basemonth=" + basemonth, "", "", false);
			 //todo : 트랜잭션이 일어난 데이터만 저장하기
			 mySheet.DoSave("main.scr.main_scr_1051.do" ,{ Param : "hidden_key=2&basemonth=" + basemonth });
			 //if(!isSuccess) return;
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=기타조정손익등록&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=기타조정손익등록&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=기타조정손익등록&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
