...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "조회":                 // 조회
			if(sjgubun == 99)
            {
            	alert("실적구분 선택후 조회하세요");
              	return;
            } 

			if(basemonth < "201201" )
            {
            	alert("외환실적 조정등록은 2012년부터 조회 가능합니다.");
              	return;
            } 
            mySheet.DoSearch("main.scr.main_scr_1311.do?hidden_key=9&basemonth=" + basemonth + "&sjgubun=" + sjgubun); 
			showUnitText("원");
			break;
...
*/			
        ...
        case "저장":                    // 저장
            ...
            //var isSuccess = mySheet.DoSave("main.scr.main_scr_1311.do?hidden_key=2&basemonth=" + basemonth + "&sjgubun=" + sjgubun, "", "", false);
			var isSuccess = mySheet.DoSave("main.scr.main_scr_1311.do", {Param : "?hidden_key=2&basemonth=" + basemonth + "&sjgubun=" + sjgubun});
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
	if (chk== 1) {
    	hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=외환실적조정등록&basemonth="+basemonth));
       	//InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       	hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=외환실적조정등록&basemonth="+basemonth));
       	//InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       	hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=외환실적조정등록&basemonth="+basemonth));
       	//InputButtControl(1);
    }
}

// 확정취소(btn_list1) 사용자권한이 전산정보부 성과/금기부성과(450/400)권한이고 완료(1) 상태 일때만 표시
...
/*--------------------------------------------*/
/* @기능 : 선택된 실적구분에 따른 그리드 init */
/*--------------------------------------------*/        
function initGrid()
{   
	v_sjgubun = document.frm.sjgubun.value; 
   	    
    if (v_sjgubun == '1')   setGridOverdue();       //'coporate deal'
    //else if (v_sjgubun == '2')  setGridImprovement();               
}

/*-----------------------------------------------------*/
/* @기능 : 실적구분(coporate deal)에 따른 Grid set     */
/*-----------------------------------------------------*/   
function setGridOverdue()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|점번호|실적구분|조정실적", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_1",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_2",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_3",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
             {Type:"Float",     Hidden:0,  MinWidth:150,  Align:"Right",   SaveName:"v_sv_4",    CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   InsertEdit:0 } ];
