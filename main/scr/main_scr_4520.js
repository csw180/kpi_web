...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                 // 조회
             mySheet.DoSearch("main.scr.main_scr_4521.do?hidden_key=9&basemonth="+basemonth+"&jumcode="+jumcode); 
             break;
        case "엑셀내려받기":            // 엑셀내려받기
             inqText       = new Array();
             inqText[0]    = new Array();
...
*/
        ...
        case "초기화":                  // 초기화
             ...
             break;   
        case "저장":                    // 저장
             var Row = mySheet.ColValueDup("1|2|3|4|5|6");
             if(Row>-1)
             {
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 부지점장목표 데이터는 삭제되고, \n\n" + 
                                              "Excel 업로드된 데이터로 반영되게됩니다.\n\n등록하시겠습니까?"))
             {
                return;
             }
             
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_4521.do?hidden_key=1&basemonth=" + basemonth, "", "", false); 
             var isSuccess = mySheet.DoSave("main.scr.main_scr_4521.do", {Param : "hidden_key=1&basemonth=" + basemonth}); 
             if(!isSuccess) return;
             break;
    }
}
...
function setGrid2014()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	let strHeader="상태|작업기준년월|직원번호|점번호|KPI코드|사업부소속코드|< 기준실적 >\n 상품분류코드" +
   		"|연간목표" +
   		"|반기1st월목표|반기2nd월목표|반기3rd월목표|반기4th월목표|반기5th월목표|반기6th월목표";
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_6",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:7 },
