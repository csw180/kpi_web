...
function LoadPage()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|고객번호|<변경전>\n사업부소속코드|<변경후>\n사업부소속코드|<변경후>\n사업부분류코드|등록사유및근거|등록직원번호", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_sv_2",    KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:9 },
             {Type:"Int",       Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_3",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:1 },
             {Type:"Int",       Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_4",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:1 },
             {Type:"Int",       Hidden:0,  MinWidth:100,  Align:"Right",   SaveName:"v_sv_5",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:2 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Left",    SaveName:"v_sv_6",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:12 },
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Left",    SaveName:"v_sv_7",    CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:10 } ];
    
   	mySheet.InitColumns(cols);

   	mySheet.SetEditable(0);
   	mySheet.SetVisible(1);
	mySheet.SetToolTipText(0,2,"조회된 셀에 잠시 마우스를 머무르면 실명번호가 표시됩니다.");
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                    // 조회
             mySheet.DoSearch("main.scr.main_scr_1121.do?hidden_key=9&basemonth=" + basemonth); 
             break;
        ...
        case "저장":                    // 저장
             ...
             /*for(i=1; i<=mySheet.RowCount; i++)
             {
                for(j=1; j<=7; j++)
                {
                   ...
                   if(mySheet.CellValue(i,j) == "" || mySheet.CellValue(i,j) == "0")
                   {
                      alert('[' + i + ']번째의 Row의 정보중 미입력 값이 존재합니다\n\n' +      
                            '확인 후 다시 Excel 업로드하시기 바랍니다.');
                      mySheet.SelectCell(i, j);
                      return;
                   }
                }
             }*/

             //변경후사업부소속코드 -> 변경후사업부분류코드 의 시작 숫자 같아야 함.             
             for(i=1; i<=mySheet.RowCount(); i++)
             {                  
                if(mySheet.GetCellValue(i,4).substr(0,1) != "1" && mySheet.GetCellValue(i,4).substr(0,1) != "2"){
                  alert('[' + i + ']번째의 Row의 변경후사업부소속코드가 올바르지 않습니다.\n\n'+
                        '확인 후 다시 Excel 업로드하시기 바랍니다.');
                  mySheet.SelectCell(i, "4");
                  return;   
                }
                else if(mySheet.GetCellValue(i,4).substr(0,1) != mySheet.GetCellValue(i,5).substr(0,1))
                {
                  
                  alert('[' + i + ']번째의 Row의 입력하신 변경후사업부분류코드는\n\n' + 
                        '변경후사업부소속코드에 포함되지 않습니다.\n\n'+
                        '확인 후 다시 Excel 업로드하시기 바랍니다.');
             ...
             var isSuccess = mySheet.DoSave("main.scr.main_scr_1121.do", {Param : "hidden_key=2&basemonth=" + basemonth }); 
...
function selectcommit(chk)
{
    ...
    // 확정구분 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=사업부변경고객등록&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=사업부변경고객등록&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=사업부변경고객등록&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
