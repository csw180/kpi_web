...
'주  시 스 템 : 성과 관리시스템
'서브  시스템 : 운영관리 - 본부데이타입력
'프로그램 ID  : main_scr_4010.jsp
'프로그램 명  : 개인별PI권유직원 변경
'프로그램개요 : 개인별PI 권유직원번호 및 실명번호 변경
'작   성   자 : 조형석
'작   성   일 : 2008.06.20
====================================================================
'수정자/수정일 :
...
<script language="javascript" src="/kpi/main/scr/main_scr_4010.js"></script>
...
function mySheet_OnLoadExcel() {
	for(i=1; i<=mySheet.RowCount("I"); i++) {
    	v_ijagbn = mySheet.GetCellValue(i, 5);
        v_pygbn  = mySheet.GetCellValue(i, 4);
        mySheet.SetCellValue(i, 5) = v_ijagbn.toUpperCase();
        mySheet.SetCellValue(i, 4) = v_pygbn.toUpperCase();
        
         if(mySheet.GetCellValue(i, 5) == "Y") {
			if(mySheet.GetCellValue(i, 2) == "0" || mySheet.GetCellValue(i, 2)== "") {
           		mySheet.SetCellValue(i, 7, mySheet.GetCellValue(i, 7) +" 계정코드 미입력" );
               	fchk = true;
            }   
         } else if(mySheet.GetCellValue(i, 5) == "N") { 
			if(mySheet.GetCellValue(i, 6) == "0" || mySheet.GetCellValue(i, 6)== "") {
           		mySheet.SetCellValue(i, 7, mySheet.GetCellValue(i, 7) +" 평가대상구분 미입력");
               	fchk = true;
         	}
		}      
	}   
...
function mySheet_OnSearchEnd(Code, Msg) {    
    ...
    var fRow = mySheet.FindText(1, document.frm.accunt.value,0);
...
function mySheet_OnClick(Row,Col,Value) {
	...
	if (v_inChk != 1) {
	    ...
	    //계좌번호
	    document.frm.accunt.value     = mySheet.GetCellValue(Row, 1);
	    //계정과목코드
	    document.frm.accuntcode.value = mySheet.GetCellValue(Row, 2);
	    //권유자직원번호
	    document.frm.gusabun.value    = mySheet.GetCellValue(Row, 3);
	    //원장변경여부
	    document.frm.wonjangch.value  = mySheet.GetCellValue(Row, 4);
	    //이자여부
	    document.frm.ijagbn.value     = mySheet.GetCellValue(Row, 5);
	    //평가대상구분
	    document.frm.pgagbn.value     = mySheet.GetCellValue(Row, 6);
	    document.frm.accuntname.value = "";
	    if (document.frm.ijagbn.value == "Y") selectaccuntname(1);
	}    
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabletype2 pdLR mgT5">
  	<tr>
	    ...
	    <th>계정과목코드</th>
	    <td >
        	<input type="text" class="input_l" onchange="selectaccuntname()" name="accuntcode" size="8" >
        	<input type="text" class="input_l" readonly name="accuntname" size="18">  
    	</td>
    	<th>권유자직원번호</th>
    	<td class="end">
        	<input type="text" name="gusabun" class="input_l" onkeyup="chk_num()" size="10">
    	</td>
  	</tr>
  	<tr>
	    <th class="nobor">원장변경여부</th>
	    <td class="nobor">
	        <select name="wonjangch" class="select">
	        	<option value="Y">Y</option>
	            <option value="N">N</option>
	        </select>
	    </td>  
    	<th class="nobor">이자여부</th>
    	<td class="nobor">
	        <select name="ijagbn" class="select">
	        	<option value="Y">Y</option>
	            <option value="N">N</option>
	        </select>
    	</td>
    	<th class="nobor">평가대상구분</th>
    	<td class="nobor end">
	        <input type="text" name="pgagbn" class="input_l" onkeyup="chk_num()" size="2">
	        <a href="javascript:popupTOTCode('P036')"><img src="/kpi/img/sky/butt_search_s.gif" border="0" align="absmiddle"></a>
    	</td>  
  	</tr>
</table>
