...
function LoadPage()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|점번호|대손충당금", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Int",       Hidden:0,  MinWidth:130,  Align:"Right",   SaveName:"v_sv_3",    CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:12 } ];
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":                    // 조회
             mySheet.DoSearch("main.scr.main_scr_1151.do?hidden_key=9&basemonth=" + basemonth); 
             break;
...
/*                if(mySheet.CellValue(i, 3) < 31 || mySheet.CellValue(i, 3) > 48 ) 
             ...
             var isSuccess = mySheet.DoSave("main.scr.main_scr_1151.do", { Param : "hidden_key=2&basemonth=" + basemonth });
...
function selectcommit(chk)
{
    ...
    // 확정구분 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=대손충당금등록&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=대손충당금등록&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=대손충당금등록&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
