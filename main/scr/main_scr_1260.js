...
function LoadPage()
{
    ...
    // 상위 GRID에 대한 속성정보 설정
   	...
   	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"상태|작업기준년월|점번호|연체대상구분|고객구분|고객번호|성명|차감연체금액", Align:"Center"} ];
   	...
   	var cols = [ {Type:"Status",    Hidden:0, MinWidth:100,  Align:"Center",  SaveName:"v_status",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
             ...
             {Type:"Text",      Hidden:0,  MinWidth:100,  Align:"Center",  SaveName:"v_gubun",   KeyField:1,   CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:5 },
...
*/
        ...
        case "저장":                    // 저장
             ...
             if(rowArray.length-1>0 && !confirm("기존 " + basemonth.substring(0,4) + "년 " + basemonth.substring(4,6) + "월의 모든 점별연체금액조정 데이터는 삭제되고, \n\n" + 
             ...
             //var isSuccess = mySheet.DoSave("main.scr.main_scr_1261.do?hidden_key=2&basemonth=" + basemonth, "", "", false);
			var isSuccess = mySheet.DoSave("main.scr.main_scr_1261.do", {Param : "hidden_key=2&basemonth=" + basemonth });
...
function selectcommit(chk)
{
    ...
    // 확정여부 조회
    if (chk== 1) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=1&v_commit=0&scrName=점별연체금액조정&basemonth="+basemonth));
       //InputButtControl(1);
    // 확정 시킴
    } else if (chk== 2) {
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=1&scrName=점별연체금액조정&basemonth="+basemonth));
       //InputButtControl(3);
    // 확정취소 시킴
    } else if (chk== 3) {    
       hiddenGrid.DoSearch(encodeURI("comm.scr.comm_scr_9098.screen?hidden_key=2&v_commit=2&scrName=점별연체금액조정&basemonth="+basemonth));
       //InputButtControl(1);
    }
}
