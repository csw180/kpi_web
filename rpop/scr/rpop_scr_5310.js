...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "고정필드설정":
        	...
        	ufSetMergeCell(mySheet, mySheet.HeaderRows(), 0, 1,5);
        	break;
        case "고정필드해제":
        ...
        case "기준년월":         // 조회(상위 GRID)
            hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&v_inqtype=1&jekyocode=3&stmonth=202007");         
            break;
        case "조회":             // 조회(상위 GRID)
            
            mySheet.DoSearch("rpop.scr.rpop_scr_5310.do?hidden_key=9&basemonth="+basemonth+"&pgcode="+pgcode+"&sergb="+sergb); 
...
function initGrid()
{
    basemonth   = document.frm.basemonth.value;
         ...
         if(basemonth >= '202007') setGrid2020H2();
...
{	
  ...
  // 상위 GRID에 대한 속성정보 설정
	...
	var strHeader1 = 
  		"점번호|점명|PG명|총금액|총금액(A)\n(월중평잔적용)|"                              //5
  		+"소계|금융기관성|본부조달|임직원가족|단기신규해지|본부마케팅|"                    //6 조정내역(B)
  		+"독도지점교차실적|신상품가중치|평가제외|기타조정|"                                //4 조정내역(B)
  		+"누증실적\n(C)=(A+B)|저원가성\n추가인정(D)|최종 누증실적\n(E)=(C+D)|기준실적(F)|" //4
  		+"순증실적\n(G)=(E-F)|순증목표\n(H)|달성률\n(I)=(G/H)"                             //3
  		;
