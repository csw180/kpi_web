...
function chk_jum() 
{
	...
	{ 
        ...
        if (chst =='0' || chst == 'R' || chst == '1')  
		{
			...
			alert("점번호는 0~, 1~ 또는 R~ 만 가능합니다. 점번호를 확인하세요" );
	}
}
...
function doAction(sAction)
{
    ...
    if (basemonth >= '202107')  pdtgubun = '0'; // 전체 조회
    ...
    switch(sAction)
    {
        ...
        case "조회":             // 조회(상위 GRID)
            ...
            mySheet.DoSearch("rpop.scr.rpop_scr_7260.do?hidden_key=9&basemonth="+basemonth+"&v_jumcode="+v_jumcode+"&pdtgubun="+pdtgubun+"&bgubun="+bgubun+"&mgubun="+mgubun); 
            break;
		...
		case "고객별실적조회":
			var basemonth = document.frm.basemonth.value;
		  	...
		  	var url = "kpi.rpop.rpop_7270.screen?basemonth="+basemonth+"&jumcode="+jumcode+"&custgubun=100"+"&jumname="+jumname+"&IS_POPUP=Y&SCREEN_URL=kpi.rpop.rpop_7270.screen";
...
function setGrid2021H2()
{
    ...
    //SetActionMenu("엑셀내려받기|고객별실적조회");//rp_op_3030 연체고객별보고서 조건
