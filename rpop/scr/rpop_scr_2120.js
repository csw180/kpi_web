...
function doAction(sAction)
{
    ...
    isSelected  = true;
    basemonth   = document.frm.basemonth.value;
    vjumcode    = document.frm.jumcode.value;
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            ...
            break;
        case "조회":             // 조회(상위 GRID)
            mySheet.DoSearch("rpop.scr.rpop_scr_2120.do?hidden_key=9&basemonth="+basemonth+"&v_jumcode="+v_jumcode+"&excsubpdt="+excsubpdt); 
...
function call1020()
{
	f = document.frm;
  	...
  	jumname   = encodeURI(f.jumname.value);
  	pgcode    = f.pgcode.value;
  	pgname    = encodeURI(f.pgname.value);
...
function initGrid()
{
    ...
    if (basemonth <= '201306')  setGrid2013Q2();
    else setGrid2013Q3(); 
}

/*-----------------------------------------*/
/* @기능 : 2013년도 상반기 기준 그리드 set */
/*-----------------------------------------*/        
function setGrid2013Q2()
{
  //mySheet = document.frm.mySheet;
	...
	createIBSheet2(  document.getElementById("sheetObj"),"mySheet", "100%", getDefaultHeight($("#sheetObj")));
  	...
  	var headers = [ { Text:"구분|당월실적|당월실적|당월실적|전월실적|전월실적|전월실적|전반기실적|전반기실적|전반기실적|개선도\n[(1+C)-(1+C')]\n/(1+C')|고객구분", Align:"Center"},
              { Text:"구분|평잔\n(A)|이익\n(B)|NIM\n(C)=(B)/(A)|평잔|이익|NIM|평잔\n(A')|이익\n(B')|NIM\n(C')=(B')/(A')|개선도\n[(1+C)-(1+C')]\n/(1+C')|고객구분", Align:"Center"} ];
  	mySheet.InitHeaders(headers, info);

  	var cols = [ {Type:"Text",     Hidden:0,  MinWidth:180,  Align:"Left",    ColMerge:1,   SaveName:"col1" },
         ...
         {Type:"Int",       Hidden:0,  MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
         {Type:"Float",     Hidden:0,  MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Float",       PointCount:5,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
         {Type:"Int",       Hidden:0,  MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
         {Type:"Int",       Hidden:0,  MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
         {Type:"Float",     Hidden:0,  MinWidth:100,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Float",       PointCount:5,   UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
         {Type:"Int",       Hidden:0,  MinWidth:120,  Align:"Right",   ColMerge:1,   SaveName:"",      CalcLogic:"",   Format:"Integer",     PointCount:-1,  UpdateEdit:0,   InsertEdit:0,   EditLen:18 },
...
/*-----------------------------------------*/
/* @기능 : 2013년도 하반기 기준 그리드 set */
/*-----------------------------------------*/        
function setGrid2013Q3()
{
    //mySheet = document.frm.mySheet;
	if(typeof(mySheet) !== "undefined")
	{
  	...
  	createIBSheet2(  document.getElementById("sheetObj"),"mySheet", "100%", getDefaultHeight($("#sheetObj")));
   	...
   	mySheet.SetConfig( { SearchMode:2, MergeSheet:5, Page:20 } );

   	var info    = { Sort:0, ColMove:1, ColResize:1, HeaderCheck:1 };
   	var headers = [ { Text:"구분|당월실적|당월실적|당월실적|전월실적|전월실적|전월실적|고객구분", Align:"Center"},
                 { Text:"구분|평잔\n(A)|이익\n(B)|NIM\n(C)=(B)/(A)|평잔|이익|NIM|고객구분", Align:"Center"} ];
   	mySheet.InitHeaders(headers, info);
