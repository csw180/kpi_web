...
function LoadPage() {
	...
	if (getUrlParameter ('IS_POPUP') == "Y") {
 	...
 	//지원안함[check again] if (location.hostname != "") InitHostInfo(location.hostname, location.port, page_path);
  	...
  	var info    = { Sort:1, ColMove:1, ColResize:1, HeaderCheck:1 };
  	var headers = [ { Text:"고객명|고객번호|고객구분|상품명|당월말누적|당월말누적|당월말누적|전월말누적|전월말누적|전월말누적|증감|증감|증감", Align:"Center"},
              { Text:"고객명|고객번호|고객구분|상품명|대출잔액|연체잔액|연체평잔|대출잔액|연체잔액|연체평잔|대출잔액|연체잔액|연체평잔", Align:"Center"} ];
  	...
  	var cols = [ {Type:"Text",     Hidden:0,  MinWidth:130,   Align:"Left",    ColMerge:1,   SaveName:"col1" },
         ...
         {Type:"Text",     Hidden:0,  MinWidth:260,  Align:"Left",    ColMerge:1,   SaveName:"col4" },
...
function selectmonth(){
    ...
    hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&basemonth="+basemonth+"&v_inqtype=2&jekyocode=8");    // 고객구분
	//자동 trim - on
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "고정필드설정":
        	...
        	ufSetMergeCell(mySheet, mySheet.LastRow(), 0, 1,4);
        	break;
        case "고정필드해제":
        ...
        case "조회":             // 조회(상위 GRID)
            ...
            mySheet.DoSearch("rpop.scr.rpop_scr_3030.do?hidden_key=9&basemonth="+basemonth+"&v_jumcode="+v_jumcode+"&custgubun="+custgubun+"&searchCode="+searchCode+"&pg_url="+pg_url);
