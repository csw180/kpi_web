...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "기준년월":             // 조회(상위 GRID)
            if (monthRole != "true") {
            	 hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&v_inqtype=1&jekyocode=6&stmonth=201101"); 
            }
            else { 
            	 hiddenGrid.DoSearch("comm.scr.comm_scr_9099.screen?hidden_key=9&v_inqtype=1&jekyocode=9&stmonth=201101"); 
            }
            break;
        ...
        case "조회":             // 조회(상위 GRID)
            if(basemonth < "201101" )
            {
              alert("종합이익증대평가는 2011년도 부터\n\n 조회가능합니다");
              return;
            }
            mySheet.DoSearch("rpop.scr.rpop_scr_5060.do?hidden_key=0&basemonth="+basemonth); 
...
{  
    ...
    if (basemonth >= '201701') setGrid2017();
    else setGrid();
}
...
function setGrid2017()
{
  //mySheet = document.frm.mySheet;
	...
	createIBSheet2(  document.getElementById("sheetObj"),"mySheet", "100%", "790px");
  	basemonth   = document.frm.basemonth.value;
	...
	for(col=15; col<15; col++ ){
  		mySheet.SetGetCellAlign(0,col,"CenterTop");
  	}
  	...
  	var cols = [ {Type:"Text",      Hidden:0,  MinWidth:70,   Align:"Center",  ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
         {Type:"Text",      Hidden:0,  MinWidth:70,   Align:"Center",   ColMerge:1,   SaveName:"",  CalcLogic:"",   Format:"",            PointCount:-1,  UpdateEdit:0,   InsertEdit:0 },
