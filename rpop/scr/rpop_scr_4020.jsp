...
'주  시 스 템 : 성과 관리시스템
'서브  시스템 : 영업점보고서 - 영업점총괄
'프로그램 ID  : rpop_scr_4020.jsp
'프로그램 명  : 영업점 총괄세부실적Ⅰ
'프로그램개요 : 영업점 총괄세부실적Ⅰ (수익성,건전성,고객관계)
'작   성   자 : 변휘원
'작   성   일 : 2006.04.12
====================================================================
...
<%  
    ...
    String      htm_title        = "영업점 총괄세부실적Ⅰ (수익성,건전성,고객관계)"; 
    String      actionname       = "";
    String      haengwon_no      = "";
    String      strErrMsg        = "";
    String      status           = "0";
...
<script language="javascript" src="/kpi/rpop/scr/rpop_scr_4020.js"></script>
...
<script>
	function mySheet_OnMouseDown(Button, Shift, X, Y)
	{
		...
		Col=mySheet.MouseCol();
		  //19년 하반기 기준 평점 숨김설정
		if (document.frm.basemonth.value >= '201907' && document.frm.basemonth.value <= '201912') {
			if(Row == 1 && mySheet.GetCellValue(1,Col).substring(0,4) == "전략평점") {
			    if(mySheet.GetColHidden(Col+4)){
			      // hidden 풀기
			      // 전략사업 비이자
			      mySheet.SetColHidden(Col+18,0);
			      mySheet.SetColHidden(Col+17,0);
			      mySheet.SetColHidden(Col+16,0);
			      mySheet.SetColHidden(Col+15,0);
			      // 전략사업 기업
			      mySheet.SetColHidden(Col+11,0);
			      mySheet.SetColHidden(Col+10,0);
			      // 전략사업 소매
			      ...
			      // header 색상 정의      
			      // 전략사업 비이자
			      mySheet.SetCellBackColor(1,Col+18,"#FFEFBF");
			      mySheet.SetCellBackColor(1,Col+17,"#FFEFBF");
			      mySheet.SetCellBackColor(1,Col+16,"#FFEFBF");
			      mySheet.SetCellBackColor(1,Col+15,"#FFEFBF");
			      // 전략사업 기업
			      mySheet.SetCellBackColor(1,Col+11,"#FFEFBF");
			      mySheet.SetCellBackColor(1,Col+10,"#FFEFBF");
			      // 전략사업 소매
			      ...
			      mySheet.SetCellText(1, Col ,"전략평점(☜)");//+☜◀
			      // 전략사업 비이자
			      mySheet.SetColHidden(Col+18,1);
			      mySheet.SetColHidden(Col+17,1);
			      mySheet.SetColHidden(Col+16,1);
			      mySheet.SetColHidden(Col+15,1);
			      // 전략사업 기업
			      mySheet.SetColHidden(Col+11,1);
